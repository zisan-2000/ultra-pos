...
export default async function FeatureAccessRequestsPage() {
  ...
  const canView = hasPermission(user, "view_feature_access_requests");
  const canManage = hasPermission(user, "manage_feature_access_requests");

  if (!canView && !canManage) {
    redirect("/dashboard");
  }

  const pendingRequests = await prisma.featureAccessRequest.findMany({
  ...
  const recentRequests = await prisma.featureAccessRequest.findMany({
